...
Attempting this project as a web developer without a math or data science background was a long shot, and I went into it with only the expectation that I'd learn a lot from trying. I got better results than I thought was possible, 

## Background

This a comic graded and encapsulated by CGC, which was the first comic book grading company and remains the most popular. The only jargon I'll use is that these are "slabbed" comics, as opposed to a regular "raw" comic.
...
After trying a few popular models I began to suspect they were essentially cheating by spitting out the "average" grade based on the comic's age. To test that I gave them some high-grade old comics, and a couple low-grade modern comics. Prompt: "As a professional grader, use the provided scans of the front and back cover to assign a grade from 0.5 to 10." Prompting to describe all the defects *before* assigning a grade seemingly doesn't help.
```{python}
...
I scraped what is likely a majority of high-resolution images of graded comics
...
- v2: Non-ML methods using [OpenCV](https://opencv.org/) often failed because of several boundaries close together, or on comics the same color as the background
- v3: A [DETR](https://huggingface.co/docs/transformers/en/model_doc/detr) object detection model. Requires manually labeling some images with bouding boxes. 
- v4: Training a U-Net model on top of a segmentation backbone to produce one heatmap for each corner and converting those into points. A bounding box is created as (x_min, y_min, x_max, y_max)
- v4.1: Correcting for rotation and perspective using OpenCV
...
I found the FastAI defaults (which haven't changed much in 7 years) very difficult to beat. There's a fair amount of randomness in training, so even if I could get a slight improvement it's hard to say whether it was just luck.
